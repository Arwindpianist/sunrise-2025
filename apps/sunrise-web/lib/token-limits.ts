...
export type BulkGuidance = {
  shouldRecommendBulk: boolean
  recommendedProgram?: "commit_s" | "commit_m" | "commit_l"
  reason?: string
}
...
export function getRecommendedUpgrade(
  ...
  if (tier === 'basic' && (limit === -1 || tokensUsed >= limit * 0.8)) return 'pro'
  if (tier === 'pro' && (limit === -1 || tokensUsed >= limit * 0.8)) return 'enterprise'
  return undefined
}
...
export function getUpgradePromptMessage(
  ...
  switch (reason) {
    case 'limit_reached':
      if (tier === 'free') {
        return `You've reached your free plan token limit of ${limit} tokens. Upgrade to Basic to continue sending.`
      } else if (tier === 'basic') {
        return `You've reached your Basic plan token limit of ${limit} tokens. Upgrade to Pro for better token value.`
      }
      return `You've reached your ${tier} plan token limit of ${limit} tokens. Upgrade for better value and higher allowance.`
    
    case 'near_limit':
      const remaining = limit - currentBalance
      return `You're running low on tokens (${remaining} remaining). Consider upgrading for lower token costs.`
    
    case 'insufficient_tokens':
      return `You don't have enough tokens for this action. Buy more tokens or upgrade for better rates.`
    
    default:
      return 'Consider upgrading for lower token rates and more included tokens.'
  }
}
...
export function calculateOptimalPurchaseAmount(
  ...
  if (limit === -1) {
    return { amount: desiredAmount, reason: 'This plan allows flexible token top-ups.' }
  }
  ...
  if (remainingSpace <= 0) {
    return { 
      amount: 0, 
      reason: `You've reached your ${tier} plan limit. Consider upgrading for higher allowance and better value.` 
    }
  }
  ...
  return { 
    ...
    reason: `Limited to ${remainingSpace} tokens to stay within your ${tier} plan limit.` 
  }
} 

export function getBulkProgramGuidance(monthlyTokenDemand: number): BulkGuidance {
  if (monthlyTokenDemand >= 50000) {
    return {
      shouldRecommendBulk: true,
      recommendedProgram: "commit_l",
      reason: "Your volume is a strong match for a high-volume commit contract.",
    }
  }
  if (monthlyTokenDemand >= 10000) {
    return {
      shouldRecommendBulk: true,
      recommendedProgram: "commit_m",
      reason: "A mid-size monthly commit can reduce overage exposure at your volume.",
    }
  }
  if (monthlyTokenDemand >= 2000) {
    return {
      shouldRecommendBulk: true,
      recommendedProgram: "commit_s",
      reason: "A starter bulk commit can stabilize costs for recurring high-volume sends.",
    }
  }
  return { shouldRecommendBulk: false }
}