...
export async function POST(request: Request) {
  try {
    ...
    const { tier, pricingProgram, bulkProgram } = await request.json()
    ...
    const checkoutParams: Stripe.Checkout.SessionCreateParams = {
      ...
      metadata: {
        ...
        pricing_model: "weighted_tokens_v2",
        target_margin_floor: "0.55",
        pricing_program: pricingProgram || "standard",
        bulk_program: bulkProgram || "none",
      },
      subscription_data: {
        metadata: {
          user_id: session.user.id,
