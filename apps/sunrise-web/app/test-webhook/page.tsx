...
export default function TestWebhookPage() {
  ...
  const runTest = async (testType: string) => {
    ...
    // Add confirmation for subscription creation tests
    if (testType === 'create_subscription' || testType === 'full_test') {
      const confirmed = window.confirm(
        '⚠️ WARNING: This will create a subscription WITHOUT payment!\n\n' +
        'This is for testing purposes only. The subscription will be created directly in the database without going through Stripe.\n\n' +
    ...
    try {
      const response = await fetch('/api/subscription/test-webhook', {
        ...
        body: JSON.stringify({
          userId: user.id,
          testType,
          confirmation: (testType === 'create_subscription' || testType === 'full_test') 
            ? 'I_UNDERSTAND_THIS_CREATES_A_SUBSCRIPTION_WITHOUT_PAYMENT' 
            : undefined
        }),
      })
        ...
        <div className="flex">
          ...
          <div className="ml-3">
            ...
            <div className="mt-2 text-sm text-red-700">
              <p>
                This page is for testing webhook functionality only. The "Test Create Subscription" and "Full Test" buttons will create subscriptions 
                <strong> WITHOUT payment verification</strong>. This bypasses Stripe and should only be used for development/testing purposes.
              </p>
            </div>
          </div>
        </div>
