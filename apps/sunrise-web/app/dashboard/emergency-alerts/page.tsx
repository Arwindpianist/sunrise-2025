...
export default function EmergencyAlertsPage() {
  ...
  const fetchEmergencyAlerts = async () => {
    try {
      setIsLoading(true)
      const res = await fetch("/api/admin/emergency-alerts", { credentials: "include" })
      if (res.status === 401 || res.status === 403) {
        toast({
          title: "Access denied",
          description: "You do not have permission to view emergency alerts.",
          variant: "destructive",
        })
        setAlerts([])
        return
  ...
  const handleAcknowledgeAlert = async (alertId: string) => {
    try {
      const res = await fetch(`/api/admin/emergency-alerts/${alertId}`, {
        method: "PATCH",
        credentials: "include",
