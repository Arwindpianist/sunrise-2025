...
export default function MessagesPage() {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const userEvents = (payload.userEvents || []) as { id: string; title: string }[]
      const emailLogs = (payload.emailLogs || []) as Record<string, unknown>[]
      const telegramLogs = (payload.telegramLogs || []) as Record<string, unknown>[]
      const discordLogs = (payload.discordLogs || []) as Record<string, unknown>[]
      const slackLogs = (payload.slackLogs || []) as Record<string, unknown>[]
      ...
      const emailMessages: Message[] = emailLogs.map((log) => ({
        ...
        event_title: eventMap.get(log.event_id as string) || "Unknown Event",
  ...
  const handleResendEvent = async (eventId: string) => {
    try {
      ...
      const balRes = await fetch("/api/user/wallet", { credentials: "include" })
      const balJson = balRes.ok ? await balRes.json() : { balance: 0 }
      const balance = typeof balJson.balance === "number" ? balJson.balance : 0
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'pending':
        return (
          <Badge className="border-transparent bg-yellow-100 text-yellow-800 hover:bg-yellow-100 dark:bg-yellow-950/60 dark:text-yellow-100">
            Pending
          </Badge>
        )
      default:
              ...
              <div className="space-y-4">
                {eventMessages.map((eventMessage) => (
                  <div
                    ...
                    className="flex flex-col sm:flex-row sm:items-center justify-between rounded-lg border border-border/60 bg-muted/40 p-4 hover:bg-muted/55 transition-colors"
                  >
                      ...
                      {eventMessage.event_id !== 'webhook' ? (
                        <>
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            className="border-green-600 text-green-600 hover:bg-green-500/10 dark:border-green-500 dark:text-green-400"
