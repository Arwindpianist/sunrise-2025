...
const PhoneImport = dynamic(() => import("@/components/phone-import"), {
  loading: () => (
    <Skeleton className="h-10 w-28 rounded-md" aria-label="Loading import" />
  ),
  ssr: false,
})
import {
...
export default function ContactsPage() {
  ...
  const handleBatchEditCategory = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(payload?.error || "Failed to update contacts")
      }
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      ...
      const payload = await res.json().catch(() => ({}))
      if (!res.ok) {
        throw new Error(payload?.error || "Failed to delete contacts")
      }
        ...
        <div className="mb-4 sm:mb-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
            <div className="min-w-0 flex-1">
              <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-foreground mb-1 sm:mb-2">Contacts</h1>
              <p className="text-muted-foreground text-xs sm:text-sm md:text-base">Manage your event contacts and categories</p>
            </div>
            {contactLimitCheck && (
              <div className="flex items-center gap-2 text-xs sm:text-sm">
                  ...
                  <div className="w-16 sm:w-20 bg-muted rounded-full h-2">
                    <div 
                      className={`h-2 rounded-full transition-all ${
                        ...
                        (contactLimitCheck.currentCount / contactLimitCheck.maxAllowed) > 0.6 ? 'bg-amber-500' : 'bg-emerald-500'
                      }`}
                ...
                {!contactLimitCheck.allowed && (
                  <Button
                    size="sm"
                    variant="outline"
                    className="border-primary/40 text-primary hover:bg-primary/10 text-xs sm:text-sm"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3">
            <Dialog>
              <DialogTrigger asChild>
                <Button size="sm" className="h-12 border-0 bg-primary text-xs text-primary-foreground transition-all duration-200 hover:bg-primary/90 sm:h-10 sm:text-sm">
                  <User className="h-4 w-4 mr-2" />
                  <span className="hidden md:inline">Invite Contacts</span>
                  <span className="md:hidden">Invite</span>
                </Button>
              </DialogTrigger>
              <DialogContent className="w-[95vw] max-w-2xl max-h-[90vh] overflow-y-auto mx-auto border-border bg-popover text-popover-foreground">
                ...
                <div className="space-y-4 px-4 sm:px-0">
                  ...
                  <div className="rounded-lg border border-primary/35 bg-primary/10 p-3 sm:p-4">
                    <h3 className="mb-2 font-semibold text-foreground">📋 Quick Overview</h3>
                  ...
                  <div className="rounded-lg border border-emerald-500/35 bg-emerald-500/10 p-3 sm:p-4">
                    <h3 className="mb-2 font-semibold text-foreground">🔗 Share Your Form</h3>
                    ...
                    <div className="mb-3">
                      <h4 className="mb-2 text-sm font-medium text-foreground">📝 Pre-formatted Message</h4>
                      <div className="rounded border border-emerald-500/40 bg-card p-3 text-xs">
                        <p className="text-foreground mb-2">Copy this message to share with your contacts:</p>
                        <div className="rounded border border-border bg-muted/50 p-2 font-mono text-xs leading-relaxed text-foreground">
...
Your info will stay private and only be used if I need to contact you. Thank You ${user?.user_metadata?.full_name || ''}`
                    ...
                    <div className="text-xs text-muted-foreground">
                      <p className="mb-1 font-medium text-foreground">Share this link on:</p>
            ...
            <Dialog open={isCategoryManagerOpen} onOpenChange={setIsCategoryManagerOpen}>
              <DialogTrigger asChild>
                <Button variant="outline" size="sm" className="h-12 border-border text-foreground hover:bg-muted/50 sm:h-10 sm:text-xs sm:text-sm">
            ...
            <Dialog open={isOnboardingDialogOpen} onOpenChange={setIsOnboardingDialogOpen}>
              ...
              <DialogContent className="w-[95vw] max-w-2xl max-h-[95vh] overflow-y-auto mx-auto border-border bg-popover text-popover-foreground p-4 sm:p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <h3 className="mb-3 text-base font-semibold text-foreground sm:text-lg">Your Telegram Setup Links</h3>
                    <div className="space-y-3 max-h-64 overflow-y-auto">
                      {onboardingLinks.length === 0 ? (
                        <p className="text-sm text-muted-foreground">No onboarding links created yet.</p>
                      ) : (
                        onboardingLinks.map((link) => (
                          <div key={link.id} className="rounded-lg border border-border bg-muted/40 p-3">
                            <div className="space-y-2">
                              <div className="flex-1">
                                <h4 className="text-base font-medium text-foreground">{link.title}</h4>
...
Your info will stay private and only be used if I need to contact you. Thank You ${user?.user_metadata?.full_name || ''}`
                  ...
                  <div className="border-t pt-4">
                    {onboardingLinks.length > 0 ? (
                      <button
                        ...
                        className="mb-3 flex w-full items-center justify-between rounded-lg p-2 text-left transition-colors hover:bg-muted/50"
                      >
                      ...
                      <form onSubmit={async (e) => {
                        ...
                        <div className="space-y-2">
                          ...
                          <DateTimeLocalPicker
                            ...
                            onChange={(next) =>
                              setOnboardingFormData((prev) => ({ ...prev, expires_at: next }))
                            }
            ...
            <Dialog open={isEditLinkDialogOpen} onOpenChange={setIsEditLinkDialogOpen}>
              <DialogContent className="w-[95vw] max-w-2xl mx-auto border-border bg-popover text-popover-foreground p-4 sm:p-6">
            ...
            {filteredContacts.length > 0 && (
              <div className="flex items-center gap-2 mt-3 sm:mt-4 pt-3 sm:pt-4 border-t">
                <input
                  ...
                  className="h-4 w-4 rounded border-border bg-muted text-primary accent-primary focus:ring-2 focus:ring-primary"
                />
                <label htmlFor="select-all" className="text-xs sm:text-sm text-foreground cursor-pointer">
        ...
        {selectedContacts.size > 0 && (
          <Card className="mb-4 border border-primary/35 bg-primary/10 sm:mb-6">
            <CardContent className="p-4 sm:p-6">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {filteredContacts.map((contact) => (
              <Card
                key={contact.id}
                className={`transition-shadow hover:shadow-md ${selectedContacts.has(contact.id) ? "bg-primary/10 ring-2 ring-primary" : ""}`}
              >
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      <input
                        ...
                        className="mt-1 h-4 w-4 flex-shrink-0 rounded border-border bg-muted text-primary accent-primary focus:ring-2 focus:ring-primary"
                      />
        ...
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent className="w-[95vw] max-w-md max-h-[90vh] mx-auto border-border bg-popover text-popover-foreground overflow-y-auto">
            ...
            <form onSubmit={handleEditContact} className="space-y-4 px-4 sm:px-0 pb-4">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2 max-h-32 overflow-y-auto">
                  ...
                  {categories.length === 0 && (
                    <p className="text-sm text-muted-foreground">No categories available. Create some categories first.</p>
                  )}
                </div>
              </div>
