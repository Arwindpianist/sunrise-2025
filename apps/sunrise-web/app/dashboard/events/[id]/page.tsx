...
export default function EventDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchEvent = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(payload?.error || "Failed to fetch event")
      }
  ...
  const handleSendMessages = async () => {
    try {
      ...
      await dispatchEventChannels(id, {
    ...
    try {
      ...
      if (event.category && event.category !== "general") {
        ...
        if (!categoryError && categoryContacts && categoryContacts.length > 0) {
          ...
          getContactsQuery = getContactsQuery.in('id', contactIds)
        } else {
      ...
      await dispatchEventChannels(newEvent.id, {
        sendEmail: event.send_email,
        sendTelegram: event.send_telegram,
      })
