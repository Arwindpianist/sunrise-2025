...
export default function DiscordLogsPage() {
  ...
  const fetchDiscordLogs = useCallback(async () => {
    try {
      setLoadError(null)
      const res = await fetch("/api/dashboard/discord-logs", { credentials: "include" })
      const data = await res.json().catch(() => ({}))
      if (!res.ok) {
        throw new Error(typeof data.error === "string" ? data.error : "Failed to load Discord logs")
      }
      if (data.unavailable) {
        setUnavailable(true)
        setLogs([])
      ...
      console.error("Error fetching Discord logs:", e)
      setLoadError(e instanceof Error ? e.message : "Could not load Discord logs")
      setLogs([])
    } finally {
      setLoading(false)
    }
  ...
  }, [status, router, subscription, subscriptionLoading, fetchDiscordLogs])
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-background via-card/20 to-background">
        <div className="mx-auto max-w-md px-4 text-center">
          ...
          <h1 className="mb-2 text-xl font-semibold text-foreground">Discord Logs</h1>
          <p className="mb-4 text-muted-foreground">Discord logs are available for Pro and Enterprise users only.</p>
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-col items-start gap-4 sm:mb-8 sm:flex-row sm:items-center lg:mb-8">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-2xl font-bold text-primary sm:text-3xl">Discord Logs</h1>
            <p className="mt-1 text-sm text-muted-foreground sm:text-base">View your Discord message history</p>
          </div>
        </div>
        ...
        {loading ? (
          <Card>
            <CardContent className="p-8">
              <div className="text-center">
                <div className="mx-auto h-8 w-8 animate-spin rounded-full border-b-2 border-primary"></div>
                <p className="mt-4 text-muted-foreground">Loading Discord logs...</p>
              </div>
            </CardContent>
          </Card>
        ) : logs.length === 0 ? (
          <Card>
            <CardContent className="p-8">
              <div className="text-center">
                ...
                <h3 className="mb-2 text-lg font-medium text-foreground">No Discord messages yet</h3>
        ...
        {logs.length > 0 && (
          <div className="mt-6 text-center sm:mt-8">
            <p className="text-xs text-muted-foreground sm:text-sm">Showing the last 50 Discord messages. Older messages are automatically archived.</p>
          </div>
        )}
      </div>
    </div>
