-- Robust fix for user signup issue - Run this in your Supabase SQL editor
...
-- 2. Create missing tables and add missing columns
...
BEGIN
  ...
  -- Add missing columns to user_enquiries table
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_enquiries' AND column_name = 'updated_at') THEN
    ALTER TABLE user_enquiries ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
  END IF;
END $$;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_referrals_referred_email') THEN
  ...
  -- User enquiries indexes
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_enquiries_user_id') THEN
    CREATE INDEX idx_user_enquiries_user_id ON user_enquiries(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_enquiries_status') THEN
    CREATE INDEX idx_user_enquiries_status ON user_enquiries(status);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_enquiries_priority') THEN
    CREATE INDEX idx_user_enquiries_priority ON user_enquiries(priority);
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_balances_user_id') THEN
    CREATE INDEX idx_user_balances_user_id ON user_balances(user_id);
  END IF;
END $$;

-- 4. Add RLS policies (drop and recreate to avoid conflicts)
...
-- 7. Create triggers for updated_at columns (only if columns exist)
DO $$
BEGIN
  -- Referrals trigger
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'referrals' AND column_name = 'updated_at') THEN
  ...
  -- User enquiries trigger
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_enquiries' AND column_name = 'updated_at') THEN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_balances' AND column_name = 'updated_at') THEN
...
-- Success message
SELECT 'Robust signup fix applied successfully! New users should now be able to sign up without errors.' as status;
