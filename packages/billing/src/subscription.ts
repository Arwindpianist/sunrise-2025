...
export const SUBSCRIPTION_FEATURES: Record<SubscriptionTier, SubscriptionFeatures> = {
  ...
  basic: {
    tier: 'basic',
    name: 'Basic',
    maxTokens: -1, // Wallet-based with weighted burns
    maxContacts: -1, // Unlimited contacts
    maxEvents: 20,
    ...
    features: [
      ...
      'Top-ups at RM0.22/token',
      'Smart contact management',
      'Event scheduling',
      'Email tracking',
      'Basic email templates',
      'Telegram messaging',
      'Channel-aware token burn',
  ...
  pro: {
    ...
    features: [
      ...
      'Top-ups at RM0.18/token',
      'Advanced email templates',
      'Telegram messaging',
      'Weighted WhatsApp utility delivery',
      'Event scheduling',
      'Email tracking',
      'Priority support',
      'Custom branding',
  ...
  enterprise: {
    ...
    features: [
      ...
      'Top-ups at RM0.14/token',
      'Premium email templates',
      'Telegram messaging',
      'Weighted multichannel delivery',
      'Event scheduling',
      'Email tracking',
      'Priority support',
      'Custom branding',
...
export function canPerformAction(
  ...
  action: 'use_telegram' | 'customize_templates' | 'custom_branding' | 'use_api' | 'buy_tokens' | 'use_bulk_program'
): boolean {
  ...
  switch (action) {
    ...
    case 'use_bulk_program':
      return features.canUseBulkProgram;
    default:
      return false;
  }
}
...
export function getUpgradeRecommendation(
  ...
  switch (reason) {
    ...
    case 'tokens':
      ...
      if (userTier === 'basic') {
        return {
          recommended: 'pro',
          reason: 'Upgrade to Pro for lower token pricing and more included monthly tokens'
        };
      }
      break;
    case 'contacts':
  ...
  switch (userTier) {
    ...
    case 'basic':
      return {
        recommended: 'pro',
        reason: 'Upgrade to Pro for lower token pricing and expanded channel capabilities'
      };
    case 'pro':
      return {
...
export function getRemainingTokenAllowance(
  ...
  if (userTier !== 'basic') return -1;
  const features = SUBSCRIPTION_FEATURES[userTier] || SUBSCRIPTION_FEATURES.basic
  if (features.maxTokens === -1) return -1
  return Math.max(0, features.maxTokens - totalTokensPurchased)
}
